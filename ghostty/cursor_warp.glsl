// --- CONFIGURATION ---
vec4 TRAIL_COLOR = vec4(iCurrentCursorColor.rgb, 0.7); // slightly transparent trail
const float DURATION = 0.16; // total animation time
const float TRAIL_SIZE = 0.7; // 0.0 = all corners move together. 1.0 = max smear
const float THRESHOLD_MIN_DISTANCE = 1.5; // min distance to show trail (units of cursor height)
const float THRESHOLD_MAX_DISTANCE = 20.0; // max distance to show trail (units of cursor height), suppresses large jumps
...
const float FADE_ENABLED = 1.0; // fade gradient along the trail (tail fades out)
const float FADE_EXPONENT = 4.0; // lower = gentler fade, higher = sharper
