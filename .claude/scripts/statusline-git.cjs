...
function parseGitStatus(status) {
  let branch = 'main', modified = 0, staged = 0, deleted = 0, untracked = 0, ahead = 0, behind = 0;
  ...
  for (const line of status.split('\n')) {
    if (line.startsWith('##')) {
      ...
      if (st === ' M' || st === 'MM') modified++;
      else if (st === 'M ' || st === 'A ' || st === 'AM') staged++;
      else if (st === ' D' || st === 'D ') deleted++;
      else if (st === '??') untracked++;
    }
  }

  return { branch, ahead, behind, modified, staged, deleted, untracked };
}
...
function formatGitStatus(counts, config) {
  ...
  return {
    ...
    staged: counts.staged > 0 ? `${COLORS.GIT_STAGED}${ICONS.GIT_STAGED}${COLORS.RESET}` : '',
    modified: counts.modified > 0 ? `${COLORS.GIT_MODIFIED}${ICONS.GIT_MODIFIED}${COLORS.RESET}` : '',
    deleted: counts.deleted > 0 ? `${COLORS.GIT_DELETED}${ICONS.GIT_DELETED}${COLORS.RESET}` : '',
    untracked: counts.untracked > 0 ? `${COLORS.GIT_UNTRACKED}${ICONS.GIT_UNTRACKED}${COLORS.RESET}` : ''
  };
}
