...
function fzf_key_bindings
    ...
    function fzf-workspace-widget -d "Change to workspace repo"
        set -l result (command ls ~/workspace/conductor-pipelines/ | fzf --reverse --height 40%)
        if test -n "$result"
            cd ~/workspace/conductor-pipelines/$result
        end
        commandline -f repaint
    end
